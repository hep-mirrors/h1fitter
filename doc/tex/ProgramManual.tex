...
organisation and it follows by a more detailed explanation of most used functions.
...
Here we choose to enlist only the routines from the common target module to gudie 
...
\small\bf{steerings} & $\bullet$ steering.txt:& free PDF parameters to be varied by MINUIT \\
 ...
 & $\bullet$ ewparam.txt:& setting ElectroWeak parameters, as well as masses \\
\toprule
...
& $\bullet$ dataset\_tools.f:&  allocating bin indecies \\
& $\bullet$ error\_logging.f: &  error logging information\\
& $\bullet$ minuit\_ini.f: & initialise minuit module \\
& $\bullet$ fcn.f: & passes to minuit the $\chi^2$ to be minimize \\
...
& $\bullet$ theory\_dispatcher.f: & distribute calculation of theory prediction for a dataset  \\
& $\bullet$ dis\_sigma.f & calculate the DIS cross sections \\
...
& $\bullet$ GetDiffDisXsection.f  & calculate the diffractive DIS cross sections \\
...
& $\bullet$ dy\_cc\_sigma.f  & calculate the DY cross sections (LO) \\
...
& $\bullet$ pp\_jets\_applgrid.f   & For DY process convolution for APPLGRID  \\
& $\bullet$ ep\_jets\_fastnlo.f  &  Calculate $ep$ jets cross sections \\\cmidrule{2-3} 
...
\item  {\tt HeraFitter}: Main steering cards. Further details can be found in the appendix 
\ref{sec:herafitter}. 
...
The following types are suppprted:
\begin{itemize}
\item \tt{TheoryType = 'DGLAP' }\rm as used for collinear evolution theories. For this type,
 it follows with aother flag which sets the order of perturbative series in $\alpha_S$:
...
The evolution starting scale is set via flag  \tt{Q02}\rm, commonly used below charm mass threshold, as imposed by QCDNUM.
...
\begin{itemize}
  \item VFNS (Variable Flavour Number Schemes):
    \begin{itemize}
     \item RT-VFS  schemes                               [from Robert Thorne], \tt{HF\_SCHEME = RT, RT OPT}\rm, as well as the fast variants based on k-factors \tt{RT FAST, RT OPT FAST}
     \item Zero Mass VFNS                                 [qcdnum], \tt{ZMVFNS}\rm   
...
\begin{itemize}
  ...
  \item $\alpha_s(Q^2)$ in FFNS is 3-flavour and recommended to be set to value of 0.105 
  ...
  \item  the pole mass definition for heavy quarks is set in ABM by default, 
    the running mass definition \cite{arXiv:1011.5790v1} can be switched in 
    by setting \tt{HF\_SCHEME = FF ABM RUNM} \rm in \tt{steering.txt} \rm. 
\end{itemize}

\item \it\bf PDF parameterisation style: \rm\\
...
\begin{tabular}{ll}
 ...
 \tt   '13p HERAPDF' & -- HERAPDF-like with Buv and Bdv floated independently \\
 \tt   '10p H12000'  & -- H12000-like (D,U,Dbar,Ubar+g) \\
 \tt   'CTEQ'        & -- CTEQ-like parameterisation \\
 \tt   'CHEB'        & -- CHEBYSHEV parameterisation based on glu,sea, uval,dval evolved pdfs \\
 \tt  'LHAPDFQ0'    & -- use lhapdf library to define pdfs at starting scale and evolve with \\
local qcdnum parameters \\
...
This section is explained in the Section \ref{sec:chi2}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{{\tt Minuit} steering cards}
The minuit steering card is described as it follows, a sample file is presented 
...
the third column is  input value for the parameter.
The forth column sets the step size (usually chosen of the same order as of the error).
To note that if step size value is 0. then this parameter is FIXED.
The fifth colum sets the lower boundary of the fit parameter, 
The sixth column set the upper boundary of the fit parameter
if boundaries are not mentioned then there are no boundaries.

Only parameters that have the step size non-zero are let to vary 
...
Examples of commands taken by minuit:\\
