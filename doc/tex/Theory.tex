...
Therefore, factorisation theorem exploits these features by 
separating the short and long distances processes, so that 
structure functions can be written as a convolution product 
between calculable parts (hard scattering coefficients) and 
non-calculable parts, which are therefore parametrised (PDFs) 
...
and currently available up to NNLO in perturbation theory at the 
differential level.

Factorisation is also proving to work for
sufficiently inclusive colored final
states, such as the one-jet and dijet cross section.
In this case no fully rigorous all-order argument is really
available, but no counterexample to factorisation has ever been found either.

Even for the diffractive case, two factorisations are used here, one is the Regge factorisation
and one the collinear factorisation for the hard scattering. 

The proton PDFs are extracted from the QCD fits by a measure of 
agreement between data and theory models.
The fit procedure is common to all the processes and it consists first 
in parametrising PDFs at a starting scale  $Q^2_0$, chosen to be below the charm mass threshold. 
The PDFs are then evolved in the \fitter\ using 
...
The LO and NNLO evolutions are also provided by the QCDNUM and can also be 
...
such as $ep$ DIS process in section \ref{dis}, $pp$ Drell-Yan process in section \ref{DY}. 
For the jet cross sections calculations, \fitter\ uses  APPLGRID or FastNLO such as in section \ref{sec:theory:jets}.And in section \ref{ttbar} the $t\bar{t}$ cross sections are calculated based on the HATHOR package.
...
final state. The DIS experiments are carried either on fixed targets 
...
Variable-Flavour number (GMVFN)~\cite{RobertThorne} schemes. 
...
The Thorne-Roberts (TR) scheme is a general-mass variable flavour number scheme (GM-VFNS)
 used as default for the MTSW PDF sets. GM-VFNS smoothly connects the two regions: 
scales below the heavy quark threshold and scales much above the heavy quark threshold. However, the definition is not unique.
...
The TR scheme comes in different variants depending on the treatment across the threshold of heavy quarks: TR standard (as used in MSTW PDF sets) and TR optimal, with a smoother transition across the heavy quark region. Both of these variants are accessible within the HERAFitter package.
The calculations are available to NLO, NNLO order. However, a fast version of the scheme is available (i.e. RT FAST) by using the $k-factor$ technique. The $k-factors$ are applied at the first iteration of the minimisation process enabling the code to run fast, as the $k-factors$ are defined as ratio between massless and massive scheme, while massless scheme accessed via QCDNUM is very fast.
...
is larger than the heavy mass)and the zero-momentum subtraction renormalization scheme for graphs with heavy quark lines (if factorisation scale is smaller than the mass of the heavy quark threshold).The DGLAP kernels and PDF evolution are pure MS-Bar. Definition of Subtractions is analogous to MS-Bar. Therefore, the ACOT scheme is considered to be a minimal extension of MS-Bar scheme.
...
Within ACOT package, different variants of the ACOT scheme are available:ACOT Full, ACOT-S, ACOT
Chi,  $\overline{\text{MS}}$  at LO and NLO. For the longitudinal structure function higher order calculationas are also available. Each of these schemes have advantages and disadvantages, for example the ACOT Full though it fully takes intp account the quark masses and it reduces
to  $\overline{\text{MS}}$  in the limit of masses going to zero, it has the disadvantage of being quite slow.
Therefore the $k-factor$ technique has been addopted within the HERAfitter machinery to be able to perform QCD fits. The $k-factor$ where can be defined in two different ways. On one hand the ratio between same order calculations but massless vs massive (i.e. NLO (ZMVFNS)/NLO (ACOT). On the other hand one could speed up the calculations by defining the $k-factors$ as the ratio between LO (massless)/NLO (massive).
Both options are available in the \fitter\ package and give similar results. For convergens of the $k-factors$ usually 2-3 iterations of complete fits are needed.


The differences between TR scheme types and ACOT scheme types are nicely summarised in the figure \ref{fig:schemes}.
...
The dipole models are valid in the low-$x$ region only, where the valence quark contribution is small, of the order of 5\%. The new HERA $F_2$ data have a precision which better than 2 \%. Therefore in the \fitter\ the contribution of the valence quarks is taken from the pdf fits and added to the original 
