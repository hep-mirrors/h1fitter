...
The PDFs are then evolved in the \fitter\ using coupled, integro-differential
...
despite the inelasticity of the interaction.
...
The events otherwise look similar to normal deep inelastic events.
...
Figure~\cite{fig:diff} illustrates the kinematic variables used to describe
...
Diffractive DIS data are fitted within the \it{proton vertex factorisation}\rm approach where 
the diffractive DIS is mediated by the exchange of hard Pomeron and a secondary Reggeon.
The model supplied by the DiffDIS package provides values of the \it{reduced cross section}\rm,
$\sigma_r^D = F_2^D - y^2/(1+(1-y)^2) F_L^D$
