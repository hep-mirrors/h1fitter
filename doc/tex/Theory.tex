...
selected via namelist {\tt HF$\_$SCHEME} in the {\tt steering.txt} (ACOT ZM, ACOT FULL, S-ACOT Chi).
...
\fitter\ includes FFN scheme from ABM~\cite{openqcdrad:page} and can use as well the QCDNUM implementation.
...
is used in the $\overline{\text{MS}}$ scheme~\cite{Alekhin:runm} is implemented
...
A schematic of the Drell Yan process is shown in figure~\ref{fig:dy}. 
...
calculation involving an increased number of diagrams with additional higher order, these 
...
(storing the matrix elements on grids such that the cross sections later may be calculated
...
the second uses the APPLGRID interface. Short description of both implementations is 
given below while for details of the theoretical modules we direct the user to read the 
references of these packages provided in the description.
%
...
The fact that PDF functions factorise, allows to significantly boost calculations when 
...
This flexibility allows to perform the calculations within the phase space
...
On the other hand, one can obtain directly the NLO predictions by using 
APPLGRID or FASTNLO techniques, which rely on factorisation theorem by 
...
allowing to be used for QCD fits. 
These method are described in more details in section \ref{sec:theory:jets}
...
This sections presents various fast calculations techniques for jet production based on
...
The APPLGRID~\cite{Carli:2010rw} package allows to compute in a fast
way an estimate
...
In order to use APPLGRID tables in \fitter\ , APPLGRID package 
has to be downloaded and installed first.
In addition, \fitter\ code has to be configured with a special option 
...
The dipole picture provides an attractive approach to the virtual photon-proton
 scattering in the low $x$ region because it allows to describe both, inclusive and 
...
can be found in~\cite{Iancu:2003ge}. The free fitted parameters are the alternative scale parameter $\tilde{R}$, $x_{0}$ and $\lambda$.
%%%%
